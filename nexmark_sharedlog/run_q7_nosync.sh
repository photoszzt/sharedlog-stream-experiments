...
# TPS_PER_WORKER=(24000 28000 32000 36000 40000)
# TPS_PER_WORKER=(4000)
...
for ((idx = 0; idx < ${#TPS_PER_WORKER[@]}; ++idx)); do
    for ((w = 0; w < ${#NUM_WORKER[@]}; ++w)); do
        ...
        subdir=${DURATION}s_${WARM_DURATION}swarm_${FLUSH_MS}ms_src${SRC_FLUSH_MS}ms_comm${COMM_EVERY_MS}ms
        for mode in ${modes[@]}; do
            for ((iter=3; iter < 6; ++iter)); do
                ./run_once.sh --app ${APP} \
                    --exp_dir ./${NUM_WORKER[w]}src_3/${subdir}/${iter}/${TPS_PER_WORKER[idx]}tps_${mode}/ \
