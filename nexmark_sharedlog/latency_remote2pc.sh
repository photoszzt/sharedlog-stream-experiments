...
OUT_DIR=$SCRIPT_DIR/../pub_data/fig10_fig11/q38_rerun/remote_2pc/
mkdir -p $OUT_DIR
...
for ((i=1; i <= 2; i++)); do
  ...
  rm -rf $OUT_DIR/q${i}-180s-0swarm-100ms-src10ms/align_chkpt-* || true
done

for ((i=3; i <= 8; i++)); do
  rm -rf $OUT_DIR/q${i}-180s-0swarm-100ms-src100ms/eo-* || true
  rm -rf $OUT_DIR/q${i}-180s-0swarm-100ms-src100ms/epoch-* || true
  rm -rf $OUT_DIR/q${i}-180s-0swarm-100ms-src100ms/align_chkpt-* || true
