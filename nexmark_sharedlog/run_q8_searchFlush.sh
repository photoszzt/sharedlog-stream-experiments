    ...
    for ((j=0; j<${#FLUSH_MS[@]}; ++j)); do
        for ((idx = 0; idx < ${#TPS_PER_WORKER[@]}; ++idx)); do
            for ((w = 0; w < ${#NUM_WORKER[@]}; ++w)); do
                ...
                ./run_once.sh --app ${APP} \
                    --exp_dir ./${NUM_WORKER[w]}src_flushSameSrc/$subdir/$iter/${TPS_PER_WORKER[idx]}tps_epoch/ \
                ...
                ./run_once.sh --app ${APP} \
                    --exp_dir ./${NUM_WORKER[w]}src_flushSameSrc/$subdir/$iter/${TPS_PER_WORKER[idx]}tps_2pc/ \
