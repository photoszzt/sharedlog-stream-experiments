...
for ((iter=0; iter < 3; ++iter)); do
    for ((idx = 0; idx < ${#TPS_PER_WORKER[@]}; ++idx)); do
        for BUF_MAX_SIZE in "${BUF_MAX_SIZES[@]}"; do
            ...
            # ./run_once.sh --app ${APP} --exp_dir ./${NUM_WORKER}src_cache/${subdir}/${TPS_PER_WORKER[idx]}tps_alo/ \
            ...
            # ./run_once.sh --app ${APP} --exp_dir ./${NUM_WORKER}src_none_gm1/${subdir}/$iter/${TPS_PER_WORKER[idx]}tps_epoch/ \
