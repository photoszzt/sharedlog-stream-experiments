...
for ((iter=0; iter < 5; ++iter)); do
    for ((idx = 0; idx < ${#TPS_PER_WORKER[@]}; ++idx)); do
        for ((w = 0; w < ${#NUM_WORKER[@]}; ++w)); do
            ...
            # $WORKSPACE_DIR/research-helper-scripts/microservice_helper start-machines --use-spot-instances
            # ./run_once.sh --app ${APP} --exp_dir ./${NUM_WORKER[w]}src_generics/${DURATION}s_${WARM_DURATION}swarm_${FLUSH_MS}ms_src${SRC_FLUSH_MS}ms/$iter/${TPS_PER_WORKER[idx]}tps_alo/ \
            ...
            # ./run_once.sh --app ${APP} --exp_dir ./${NUM_WORKER[w]}src_ets2/${DURATION}s_${WARM_DURATION}swarm_${FLUSH_MS}ms/${TPS_PER_WORKER[idx]}tps_2pc/ \
