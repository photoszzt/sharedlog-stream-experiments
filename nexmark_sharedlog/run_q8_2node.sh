...
for ((iter=0; iter < 1; ++iter)); do
    for ((idx = 0; idx < ${#TPS_PER_WORKER[@]}; ++idx)); do
        for ((w = 0; w < ${#NUM_WORKER[@]}; ++w)); do
            ...
            ./run_once.sh --app ${APP} \
                --exp_dir ./${NUM_WORKER[w]}src_2node_8ins/$subdir/$iter/${TPS_PER_WORKER[idx]}tps_epoch/ \
            ...
            #     --tps ${TPS} --warm_duration ${WARM_DURATION} \
            #     --flushms ${FLUSH_MS} --src_flushms $SRC_FLUSH_MS --snapshot_s $SNAPSHOT_S
        done
    done
done
cd -
