...
echo "exp_dir: $EXP_DIR, num_producer: $NUM_PRODUCER, num_consumer: $NUM_CONSUMER, duration: $DURATION, num partition: $NUM_PARTITION, payload: $PAYLOAD, tps: $TPS"
...
ssh -q $MANAGER_HOST -- "docker service create \
    ...
    --replicas-max-per-node=1 --publish published=8090,target=8090 ubuntu:focal /src/bin/kafka_tran_process \
...
ssh -q $MANAGER_HOST -- "docker service create \
    ...
    -npar ${NUM_PARTITION} -payload /src/data/$PAYLOAD -tps $TPS"
