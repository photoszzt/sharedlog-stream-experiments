...
def load(system, experiment):
    ...
    rows = [row for row in csv.DictReader(rows, fieldnames=headers) 
            if row['del'] == 'eo' and int(row['tps']) in per_worker_in_tp]
...
def main():
    ...
    for i in range(0, len(times)):
        ...
        if i == 0:
            ax1 = ax
        ll1, = ax.plot(input_throughput, [int(row['p50']) for row in kafka], label=f'Kafka Streams p50',
                color=colors[0], marker='v')
        ll2, = ax.plot(input_throughput, [int(row['p99']) for row in kafka], label=f'Kafka Streams p99',
        ...
        ll4, = ax.plot(input_throughput, [int(row['p99']) for row in impeller], label=f'Impeller p99',
