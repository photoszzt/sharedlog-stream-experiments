...
for ((k=0; k<$NUM_INSTANCE; k++)); do
    ...
    ssh -q $MANAGER_HOST -oStrictHostKeyChecking=no -- "docker service create \
        ...
        --mount type=bind,source=$WORKSPACE_DIR/sharedlog-stream-experiments/nexmark_kafka-streams_metrics/shared-assets/jmx-exporter,destination=/usr/share/jmx-exporter \
