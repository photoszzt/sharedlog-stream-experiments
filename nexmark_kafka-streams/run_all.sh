...
for ((j=0; j<${#APP[@]}; ++j)); do
	...
	EVENTS=$(expr ${TPS} \* $DURATION)
    	...
    	./nexmark.sh --app ${APP[j]} \
    	    --exp_dir ./${APP[j]}/4src_ets/${DURATION}s_${WARM_DURATION}swarm_${FLUSH_MS}ms/${TPS}tps_${EVENTS}_alo/ \
    	...
    	./nexmark.sh --app ${APP[j]} \
    	    --exp_dir ./${APP[j]}/4src_ets/${DURATION}s_${WARM_DURATION}swarm_${FLUSH_MS}ms/${TPS}tps_${EVENTS}_eo/ \
    	    --nins 4 --nsrc 4 --serde msgp --duration $DURATION --nevents ${EVENTS} \
